...
control 'V-218739' do
  title "Both the log file and Event Tracing for Windows (ETW) for each IIS
10.0 website must be enabled."
...
process.
    ...
    In IIS 10.0, the administrator has the option of sending logging
information to Event Tracing for Windows (ETW). This option gives the
administrator the ability to use standard query tools, or create custom tools,
for viewing real-time logging information in ETW. This provides a significant
advantage over parsing text-based log files that are not updated in real time.
  ...
  desc  'check', "
    ...
    Under Log Event Destination, verify the \"Both log file and ETW event\"
radio button is selected.

    If the \"Both log file and ETW event\" radio button is not selected, this
is a finding.

    Note: \"Microsoft-IIS-Logging/logs\" must be enabled prior to configuring
this setting. More configuration information is available at:
...
this setting.
    ...
    Under Log Event Destination, select the \"Both log file and ETW event\"
radio button.
