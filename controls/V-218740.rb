...
control 'V-218740' do
  title "An IIS 10.0 website behind a load balancer or proxy server must
...
proxy.
    ...
    If the IIS 10.0 is receiving inbound web traffic through a proxy, the audit
logs must be reviewed to determine if correct source information is being
passed through by the proxy server.
    ...
    When log file is displayed, review source IP information in log entries and
...
transparently."
  ...
  tag cci: ['V-100201', 'SV-109305', 'CCI-000133']
  tag nist: ['AU-3']
end
