...
control 'V-218736' do
  title "The IIS 10.0 website session state cookie settings must be configured
to Use Cookies mode."
...
visible only within the session. ASP.NET session state identifies requests from
the same browser during a limited time window as a session and provides the
ability to persist variable values for the duration of that session.

    When using the URI mode for cookie settings under session state, IIS will
reject and reissue session IDs that do not have active sessions. Configuring
IIS to expire session IDs and regenerate tokens gives a potential attacker less
time to capture a cookie and gain access to server content.
  "
  desc  'rationale', ''
  desc  'check', "
    ...
    Under \"Cookie Settings\", verify the \"Use Cookies\" mode is selected from
...
editor, locate \"system.web/sessionState\".
    ...
    Note: If IIS 10.0 server/site is used only for system-to-system
maintenance, does not allow users to connect to interface, and is restricted to
specific system IPs, this is Not Applicable.
  "
  desc  'fix', "
