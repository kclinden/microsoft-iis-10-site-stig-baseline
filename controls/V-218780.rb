...
control 'V-218780' do
  title "Interactive scripts on the IIS 10.0 web server must have restrictive
access controls."
...
via the web service, the execution of CGI programs is not limited unless the
System Administrator (SA) or the Web Manager takes specific measures. CGI
programs can access and alter data files, launch other programs, and use the
network.
  "
  desc  'rationale', ''
  desc  'check', "
...
listed, or more restrictive permissions are assigned.
    ...
    If the permissions are less restrictive than listed above, this is a
finding.
  "
  desc  'fix', "
    Determine whether scripts are used on the web server for the subject
