...
control 'V-218764' do
  title "The IIS 10.0 website must provide the capability to immediately
...
log data is conserved for later forensic analysis.
  ...
  desc  'check', "
    ...
    Verify there are documented procedures for shutting down an IIS 10.0
...
the following steps:
    ...
    If necessary, stop the IIS 10.0 web server by selecting the web server in
the IIS 10.0 Manager.

    In the \"Actions\" pane, under \"Manage Server\", click \"Stop\".

    If there are not documented procedures with, at a minimum, the mentioned
steps for stopping a website, this is a finding.
  "
  desc  'fix', "
    Prepare documented procedures for shutting down an IIS 10.0 website in the
event of an attack. The procedure should, at a minimum, provide the following
steps:
