...
control 'V-218767' do
  title "The IIS 10.0 website must only accept client certificates issued by
...
defense-in-depth strategy."
  ...
  desc  'check', "
    ...
    If the list of CAs in the trust hierarchy does not lead to the DoD PKI Root
CA, DoD-approved external certificate authority (ECA), or DoD-approved external
partner, this is a finding.

    If HTTPS is not an available type under site bindings, this is a finding.

    If HTTPS is not an available type under site bindings, and the Web Server
ONLY communicates directly with a load balancer/proxy server with IP address
and Domain Restrictions in place, this is not a finding.
  "
  desc  'fix', "
    Follow the procedures below for each site hosted on the IIS 10.0 web server:
    ...
    Click on the binding needing a certificate and select \"Edit\", or add a
site binding for HTTPS.
